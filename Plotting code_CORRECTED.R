...
ES2 <- AllData %>% 
  select(ESETA1, INR1orbelow) 

ES2 <- ES2[complete.cases(ES2),]


ESdens <-ggdensity(ES2, x = "ESETA1", 
          add = "mean", rug = TRUE,
...
HOME2 <- AllData %>% 
  select(HOMEETA1, INR1orbelow) 

HOME2 <- HOME2[complete.cases(HOME2),]

HOMEdens <- ggdensity(HOME2, x = "HOMEETA1",
          add = "mean", rug = TRUE,
          color = "INR1orbelow", fill = "INR1orbelow", 
          xlab="HOME: MNLFA", 
          palette = c("#0073C2FF", "#FC4E07"))
...
CTS2 <- AllData %>% 
  select(CTSETA1, INR1orbelow) 

CTS2 <- CTS2[complete.cases(CTS2),]

CTSdens <- ggdensity(CTS2, x = "CTSETA1",
          add = "mean", rug = TRUE,
          color = "INR1orbelow", fill = "INR1orbelow", 
          xlab="CTS: MNLFA", 
          palette = c("#0073C2FF", "#FC4E07"))
